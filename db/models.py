...
class Order(models.Model):
    ...
    user = ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="orders")
    ...
    def clean(self):
        ...
        if self.row < 1 or self.row > max_row:
            raise ValidationError({
                "row": f"row number must be in available range: (1, rows): (1, {max_row})"
            })

        if self.seat < 1 or self.seat > max_seat:
            raise ValidationError({
                "seat": f"seat number must be in available range: (1, seats_in_row): (1, {max_seat})"
            })
